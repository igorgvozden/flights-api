...
flights.get('/', (req, res) => {
  ...
  if (!query || typeof query.date !== 'string') {
    res.status(400).json(`'date' is a required parameter and must use the following format: ${dateFormatText}`);
    return;
  }
  ...
  if (!isoDate.isValid) {
    res.status(400).json(`'date' value (${query.date}) is malformed; 'date' must use the following format: ${dateFormatText}`);
    return;
  }
